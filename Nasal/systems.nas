...
aircraft.livery.init("Aircraft/EchoAirCruiser/Models/Liveries");
var FDM ="";
var tanks=[];
var N1=[0.0,0.0];
var fuel_cutoff=[0,0];
...
#direct wheel thruster
    ...
    var bts =getprop("/controls/electric/battery-switch");
    if(wow==1){
    if(eng1==0){
    if(eng2==0){
    if(bts==1){
    var thrust=getprop("/controls/engines/engine[0]/throttle");
    setprop("/sim/model/pushback/force",thrust);
}
}
}
}
    if(wow==0 or eng1==1 or eng2==1 or bts==0){
    setprop("/sim/model/pushback/force",0.5);
}
...
settimer(update_systems,0);
    ...
    setprop("systems/electrical/outputs/mk-viii",6);
    setprop("systems/electrical/outputs/turn-coordinator",6);
    setprop("systems/electrical/outputs/transponder",6);
    setprop("systems/electrical/outputs/gps",6);
    setprop("systems/electrical/outputs/dme",6);
    setprop("systems/electrical/outputs/adf",6);
    setprop("systems/electrical/outputs/comm",6);
    setprop("systems/electrical/outputs/nav",6);
}
